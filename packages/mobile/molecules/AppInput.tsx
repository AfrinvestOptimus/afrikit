    ...
    <View>
      <TouchableWithoutFeedback
        ...
        <View
          ...
          {leftIcon && (
            <View className="items-center justify-center h-full ml-auto px-xs">{leftIcon}</View>
          )}
          <View className="flex-1">
            {FloatingLabel && (
              <Animated.Text
                style={[floatLabelStyle]}
                className="gap-xs color-dark-gray1"
                allowFontScaling={false}>
                {label}
              </Animated.Text>
            )}
          ...
          {rightIcon && (
            <View className="items-center justify-center h-full ml-auto px-xs">{rightIcon}</View>
          )}
          {inputValue.length > 0 && (
            <TouchableOpacity onPress={handleClear} className="justify-center pl-2">
      ...
      {!!error &&
        (typeof error === 'string' ? (
          <AppText size={2} color="error" weight={'regular'} align={'left'} className={'mb-5xl'}>
            {error}
          </AppText>
        ) : undefined)}
    </View>
